...
impl Orchestrator {
    ...
    pub fn run(&mut self, prog: &Program, max_rounds: usize) -> Verdict {
        ...
        for round in 0..max_rounds {
            ...
            // ...and an answer nobody has consumed is unfinished business too.
            //
            // The `unanswered` test alone is off by exactly one round, and the
            // case it misses is the one the whole mechanism exists for.
            // `benchmarks/ajave/engine-cooperation/SinBoundNeedsAnAnswer`:
            // round 0 the BMC havocs `Math.sin`, the solver claims a value
            // above 2, and it publishes a violation — which *closes* the
            // obligation, so `open` is 0. Round 1 `ranges` answers the query it
            // posted. `unanswered` is now empty and `open` is 0, so the run
            // reports, one round before the asker could act on the answer. JVM
            // replay then refutes the witness and the task scores nothing.
            //
            // Asking whether any engine is live says the same thing without
            // naming queries: an engine with new input it has not seen has
            // work left, whether that input is a `Lemma`, a discharge that
            // shrinks a CHC query, or a `Bounded` that gives k-induction a base
            // case. It is the same question the round loop already asks to
            // build its schedule, which is why it costs nothing to ask here.
            let any_live = (0..self.engines.len()).any(|i| {
                let st = &state[i];
                !st.retired
                    && (st.steps == 0
                        || st.suspended
            ...
            //
            // Gated on something still being undecided, and `open_or_unconfirmed`
            // is exactly the right question. `open` alone says no — the
            // violation occupies the obligation — which is what ended the run
            // early. But an unconfirmed violation is a *candidate*: replay may
            // refute it, and an over-approximating engine may yet prove the
            // obligation safe. That is unfinished business by any reading.
            //
            // Without the gate, an engine that suspends forever keeps a fully
            // decided task alive to `max_rounds`, paying for a re-entry per
            // round to re-derive an answer nobody needs.
            let undecided = !self.bb.open_or_unconfirmed().is_empty();
            let outstanding = !self.bb.unanswered().is_empty() || (any_live && undecided);
...
mod scheduling_tests {
    ...
    /// The seam that made the whole `Query`/`Lemma` mechanism unusable.
    ///
    /// An under-approximating engine publishes a violation, which *closes* the
    /// obligation, so `open` is 0 and the old rule reported immediately. But an
    /// unconfirmed violation is a candidate — JVM replay may refute it — and in
    /// `benchmarks/ajave/engine-cooperation/SinBoundNeedsAnAnswer` that is
    /// exactly what happens: the BMC havocs `Math.sin`, the solver claims a
    /// value above 2, and the run ends one round before the answer `ranges`
    /// published could be consumed. The task then scores nothing.
    ///
    /// So the run must continue while some engine has new input *and* something
    /// is still undecided. `open_or_unconfirmed`, not `open`, is what asks that.
    #[test]
    fn an_unconfirmed_violation_does_not_end_the_run() {
        struct Violator;
        impl Engine for Violator {
            fn id(&self) -> EngineId {
                EngineId("violator")
            }
            fn direction(&self) -> Direction {
        ...
        let reader = Scripted::new("prover", Interest::STATUS, vec![Progress::Blocked; 8]);
        let entries = reader.entries.clone();
        run_with(vec![Box::new(reader), Box::new(Violator)], 8);
        assert!(
            entries.borrow().len() >= 2,
            "the violation is a candidate, not a verdict; an engine that could \
             refute it must still get to run"
        );
    }
