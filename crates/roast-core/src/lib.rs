//! roast-core: the verification core.
//!
//! Language-agnostic on purpose -- it knows about obligations, artifacts
//! and fixpoints, not about Java. Depends on `roast-ir` (what it analyses)
//! and `roast-models` (the exception-class lookup `certify` and the
//! orchestrator need), and nothing else. No dependency on `roast-frontend`:
