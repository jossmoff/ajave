...
impl<'a> ExploreCtx<'a> {
    ...
    fn encode_char_wrapper_call(&mut self, class: &str, name: &str, args: &[Operand]) -> Term {
        ...
        match (class, name) {
            ...
            ("java/lang/Character", "isLetterOrDigit") => {
                ...
                // alpha is already 0 or 1; convert to bool
            ...
            // isJavaIdentifierStart: letter or $ or _
            ...
            // isJavaIdentifierPart: letter, digit, $ or _
    ...
    pub(super) fn encode_str_call(
        ...
        match target.name.as_str() {
            ...
            "valueOf" if !target.desc.starts_with("(L") && !target.desc.starts_with("([") => {
                ...
                let result = self.signed_bv_to_str(arg_bv);
                Some((one, Some(result)))
            }
            _ => None,
        }
    ...
    /// Encode a signed BV32 to its decimal string representation.
    /// Z3's str.from_int only handles non-negative; for negative values
    /// we prepend "-" and negate.
    fn signed_bv_to_str(&mut self, val: Term) -> Term {
    ...
    pub(super) fn encode_wrapper_str_call(
        ...
        match (target.class.as_str(), target.name.as_str()) {
            ...
            ("java/lang/Integer", "toString") => {
                ...
                let result = self.signed_bv_to_str(val);
                Some((one, Some(result)))
            }
            // Long.toString: disabled — bv32_to_int in signed_bv_to_str truncates 64-bit values
            // TODO: implement bv64_to_int for Long support
