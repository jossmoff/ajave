...
use roast_ir::{BinOp, BlockId, CmpKind, Const, Edge, Operand, Program, Rvalue, Stmt, Ty, VarId};
...
impl IState {
    ...
    pub fn eval_rvalue_typed(&self, rv: &Rvalue, var_types: &[roast_ir::VarInfo]) -> Interval {
        self.eval_rvalue_inner(rv, Some(var_types))
    }
    fn eval_rvalue_inner(&self, rv: &Rvalue, var_types: Option<&[roast_ir::VarInfo]>) -> Interval {
        match rv {
...
fn find_defining_cmp(body: &roast_ir::Body, block: BlockId, v: VarId) -> Option<(CmpKind, &Operand, &Operand)> {
...
impl WideningIntervalCpa {
    ...
    pub fn from_body(body: &roast_ir::Body) -> Self {
        let mut headers = HashSet::new();
        for block in &body.blocks {
            let succs: Vec<BlockId> = match &block.term {
                roast_ir::Terminator::Goto(t) => vec![*t],
                roast_ir::Terminator::Branch { then_, else_, .. } => vec![*then_, *else_],
                roast_ir::Terminator::Switch { cases, default, .. } => {
