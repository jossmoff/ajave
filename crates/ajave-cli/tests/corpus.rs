//! Integration tests: run the ajave binary against every task and assert the
//! current verdict.
//!
//! # What these tests enforce
//!
//! Each test asserts the verdict ajave *currently* produces, not necessarily
...
/// Run the ajave binary against the given inputs and return the verdict line.
/// Panics if ajave crashes or produces no output.
fn ajave(inputs: &[&str]) -> String {
    ...
    let out = Command::new(binary)
        .args(&abs_inputs)
        .output()
        .unwrap_or_else(|e| panic!("failed to run ajave: {e}"));
...
fn stage00_const() {
    ...
    assert_eq!(ajave(&["tasks/stage00_const", "tasks/common"]), "FALSE");
}

#[test]
fn stage01_nondet() {
    // nondetInt() with assume(x >= 4), assert x > 3. SV-COMP: true.
    assert_eq!(ajave(&["tasks/common", "tasks/stage01_nondet"]), "TRUE");
}

#[test]
fn stage02_branch() {
    // Branch always sets y >= 10, assert y >= 10. SV-COMP: true.
    assert_eq!(ajave(&["tasks/common", "tasks/stage02_branch"]), "TRUE");
}

#[test]
fn stage04_divzero() {
    // 100 / nondetInt() — zero is reachable. SV-COMP: false.
    assert_eq!(ajave(&["tasks/common", "tasks/stage04_divzero"]), "FALSE");
}
...
fn arithmetic_exception1() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/ArithmeticException1"]),
        "TRUE"
    );
}
...
fn arithmetic_exception2() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/ArithmeticException2"]),
        "FALSE"
    );
}
...
fn arithmetic_exception3() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/ArithmeticException3"]),
        "TRUE"
    );
}
...
fn branch_divide1() {
    ...
    assert_eq!(ajave(&["tasks/common", "tasks/BranchDivide1"]), "TRUE");
}
...
fn bounded_loop1() {
    ...
    assert_eq!(ajave(&["tasks/common", "tasks/BoundedLoop1"]), "TRUE");
}

#[test]
fn bounded_loop2() {
    // n in [0,3], loop runs n times; n=0 gives sum=0, assert sum>=1 fails. SV-COMP: false.
    assert_eq!(ajave(&["tasks/common", "tasks/BoundedLoop2"]), "FALSE");
}

#[test]
fn integer_arithmetic1() {
    // x in [1,100], y = x*2, assert y > 0. SV-COMP: true.
    assert_eq!(ajave(&["tasks/common", "tasks/IntegerArithmetic1"]), "TRUE");
}
...
fn integer_arithmetic2() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/IntegerArithmetic2"]),
        "FALSE"
    );
}
...
fn modulo_zero1() {
    ...
    assert_eq!(ajave(&["tasks/common", "tasks/ModuloZero1"]), "FALSE");
}

#[test]
fn nested_branch1() {
    // Inputs bounded [1,1000], result = max(x,y) >= 1. SV-COMP: true.
    assert_eq!(ajave(&["tasks/common", "tasks/NestedBranch1"]), "TRUE");
}
...
fn jbmc_assert1() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/assert1"]),
        "TRUE"
    );
}
...
fn jbmc_assert2() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/assert2"]),
        "FALSE"
    );
}
...
fn jbmc_arithmetic_exception5() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/ArithmeticException5"]),
        "TRUE"
    );
}
...
fn jbmc_arithmetic_exception6() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/ArithmeticException6"]),
        "FALSE"
    );
}
...
fn jbmc_class_cast_exception1() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/ClassCastException1"]),
        "FALSE"
    );
}
...
fn jbmc_class_cast_exception2() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/ClassCastException2"]),
        "UNKNOWN"
    );
}
...
fn jbmc_class_method1() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/Class_method1"]),
        "UNKNOWN"
    );
}
...
fn jbmc_inheritance1() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/Inheritance1"]),
        "UNKNOWN"
    );
}
...
fn jbmc_arraylength1() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/arraylength1"]),
        "UNKNOWN"
    );
}
...
fn jbmc_arithmetic_exception1() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/ArithmeticException1"]),
        "FALSE"
    );
}
...
fn jbmc_char_sequence_bug() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/CharSequenceBug"]),
        "FALSE"
    );
}
...
fn jbmc_char_sequence_to_string() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/CharSequenceToString"]),
        "UNKNOWN"
    );
}
...
fn jbmc_class_cast_exception3() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/ClassCastException3"]),
        "FALSE"
    );
}
...
fn jbmc_regex_matches01() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/RegexMatches01"]),
        "UNKNOWN"
    );
}
...
fn jbmc_regex_matches02() {
    ...
    // UNKNOWN: regex/string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/RegexMatches02"]),
        "UNKNOWN"
    );
}
...
fn jbmc_regex_substitution01() {
    ...
    // UNKNOWN: regex/string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/RegexSubstitution01"]),
        "UNKNOWN"
    );
}
...
fn jbmc_regex_substitution02() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/RegexSubstitution02"]),
        "FALSE"
    );
}
...
fn jbmc_regex_substitution03() {
    ...
    // UNKNOWN: regex/string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/RegexSubstitution03"]),
        "UNKNOWN"
    );
}
...
fn jbmc_static_char_methods01() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StaticCharMethods01"]),
        "UNKNOWN"
    );
}
...
fn jbmc_static_char_methods02() {
    ...
    // UNKNOWN: Character static methods not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StaticCharMethods02"]),
        "UNKNOWN"
    );
}
...
fn jbmc_static_char_methods03() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StaticCharMethods03"]),
        "FALSE"
    );
}
...
fn jbmc_static_char_methods04() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StaticCharMethods04"]),
        "FALSE"
    );
}
...
fn jbmc_static_char_methods05() {
    ...
    // UNKNOWN: Character static methods not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StaticCharMethods05"]),
        "UNKNOWN"
    );
}
...
fn jbmc_static_char_methods06() {
    ...
    // UNKNOWN: Character static methods not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StaticCharMethods06"]),
        "UNKNOWN"
    );
}
...
fn jbmc_string_builder_chars01() {
    ...
    // UNKNOWN: StringBuilder not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringBuilderChars01"]),
        "UNKNOWN"
    );
}
...
fn jbmc_string_builder_chars02() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringBuilderChars02"]),
        "FALSE"
    );
}
...
fn jbmc_string_builder_chars03() {
    ...
    // UNKNOWN: StringBuilder not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringBuilderChars03"]),
        "UNKNOWN"
    );
}
...
fn jbmc_string_builder_chars04() {
    ...
    // UNKNOWN: StringBuilder not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringBuilderChars04"]),
        "UNKNOWN"
    );
}
...
fn jbmc_string_builder_chars05() {
    ...
    // UNKNOWN: StringBuilder not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringBuilderChars05"]),
        "UNKNOWN"
    );
}
...
fn jbmc_string_builder_chars06() {
    // StringBuilder char access assert failure. SV-COMP: false.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringBuilderChars06"]),
        "FALSE"
    );
}
...
fn jbmc_string_compare01() {
    ...
    // UNKNOWN: string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringCompare01"]),
        "UNKNOWN"
    );
}
...
fn jbmc_string_compare02() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringCompare02"]),
        "FALSE"
    );
}
...
fn jbmc_string_compare03() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringCompare03"]),
        "FALSE"
    );
}
...
fn jbmc_string_compare04() {
    // String comparison assert failure. SV-COMP: false.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringCompare04"]),
        "FALSE"
    );
}
...
fn jbmc_string_compare05() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringCompare05"]),
        "FALSE"
    );
}
...
fn jbmc_string_constructors01() {
    ...
    // UNKNOWN: string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringConstructors01"]),
        "UNKNOWN"
    );
}
...
fn jbmc_string_constructors02() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringConstructors02"]),
        "FALSE"
    );
}
...
fn jbmc_string_constructors03() {
    ...
    // UNKNOWN: string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringConstructors03"]),
        "UNKNOWN"
    );
}
...
fn jbmc_string_constructors04() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringConstructors04"]),
        "FALSE"
    );
}
...
fn jbmc_string_constructors05() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringConstructors05"]),
        "FALSE"
    );
}
...
fn jbmc_string_contains01() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringContains01"]),
        "FALSE"
    );
}
...
fn jbmc_string_contains02() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringContains02"]),
        "FALSE"
    );
}
...
fn jbmc_string_index_methods01() {
    ...
    // UNKNOWN: string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringIndexMethods01"]),
        "UNKNOWN"
    );
}
...
fn jbmc_string_index_methods02() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringIndexMethods02"]),
        "FALSE"
    );
}
...
fn jbmc_string_index_methods03() {
    // String index methods assert failure. SV-COMP: false.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringIndexMethods03"]),
        "FALSE"
    );
}
...
fn jbmc_string_index_methods04() {
    // String index methods assert failure. SV-COMP: false.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringIndexMethods04"]),
        "FALSE"
    );
}
...
fn jbmc_string_index_methods05() {
    // String index methods assert failure. SV-COMP: false.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringIndexMethods05"]),
        "FALSE"
    );
}
...
fn jbmc_string_start_end01() {
    ...
    // UNKNOWN: string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringStartEnd01"]),
        "UNKNOWN"
    );
}
...
fn jbmc_string_start_end02() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringStartEnd02"]),
        "FALSE"
    );
}
...
fn jbmc_string_start_end03() {
    // String.startsWith/endsWith assert failure. SV-COMP: false.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringStartEnd03"]),
        "FALSE"
    );
}
...
fn jbmc_string_value_of01() {
    ...
    // UNKNOWN: string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringValueOf01"]),
        "UNKNOWN"
    );
}
...
fn jbmc_string_value_of02() {
    ...
    // UNKNOWN: string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringValueOf02"]),
        "UNKNOWN"
    );
}
...
fn jbmc_string_value_of03() {
    ...
    // UNKNOWN: string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringValueOf03"]),
        "UNKNOWN"
    );
}
...
fn jbmc_string_value_of04() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringValueOf04"]),
        "FALSE"
    );
}
...
fn jbmc_string_value_of05() {
    ...
    // UNKNOWN: string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringValueOf05"]),
        "UNKNOWN"
    );
}
...
fn jbmc_string_value_of06() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringValueOf06"]),
        "FALSE"
    );
}
...
fn jbmc_string_value_of07() {
    // String.valueOf assert failure. SV-COMP: false.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringValueOf07"]),
        "FALSE"
    );
}
...
fn jbmc_string_value_of08() {
    ...
    // UNKNOWN: string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringValueOf08"]),
        "UNKNOWN"
    );
}
...
fn jbmc_string_value_of09() {
    ...
    // UNKNOWN: string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringValueOf09"]),
        "UNKNOWN"
    );
}
...
fn jbmc_string_value_of10() {
    ...
    // UNKNOWN: string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/StringValueOf10"]),
        "UNKNOWN"
    );
}
...
fn jbmc_sub_string01() {
    ...
    // UNKNOWN: string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/SubString01"]),
        "UNKNOWN"
    );
}
...
fn jbmc_sub_string02() {
    ...
    // UNKNOWN: string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/SubString02"]),
        "UNKNOWN"
    );
}
...
fn jbmc_sub_string03() {
    ...
    // UNKNOWN: string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/SubString03"]),
        "UNKNOWN"
    );
}
...
fn jbmc_token_test01() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/TokenTest01"]),
        "UNKNOWN"
    );
}
...
fn jbmc_token_test02() {
    ...
    // UNKNOWN: string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/TokenTest02"]),
        "UNKNOWN"
    );
}
...
fn jbmc_validate01() {
    ...
    // UNKNOWN: string operations not modelled.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/Validate01"]),
        "UNKNOWN"
    );
}
...
fn jbmc_validate02() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/Validate02"]),
        "FALSE"
    );
}
...
fn jbmc_aastore_aaload1() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/aastore_aaload1"]),
        "UNKNOWN"
    );
}
...
fn jbmc_array1() {
    ...
    // UNKNOWN: array element content not tracked.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/array1"]),
        "UNKNOWN"
    );
}
...
fn jbmc_array2() {
    ...
    // UNKNOWN: array element content not tracked.
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/array2"]),
        "UNKNOWN"
    );
}
...
fn jbmc_arrayread1() {
    ...
    assert_eq!(
        ajave(&["tasks/common", "tasks/jbmc-regression/arrayread1"]),
        "UNKNOWN"
    );
}
