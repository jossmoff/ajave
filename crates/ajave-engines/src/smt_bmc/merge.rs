...
impl<'a> ExploreCtx<'a> {
    ...
    pub(super) fn merge_states_ite(&mut self, cond: Term, a: &SavedState, b: &SavedState) {
        ...
        let all_fs: HashSet<_> = a
            .field_str_arrays
            .keys()
            .chain(b.field_str_arrays.keys())
            .cloned()
            .collect();
        self.field_str_arrays.clear();
        for k in all_fs {
            match (
                a.field_str_arrays.get(&k).copied(),
                b.field_str_arrays.get(&k).copied(),
            ) {
                (Some(t), Some(e)) if t == e => {
                    self.field_str_arrays.insert(k, t);
                }
                (Some(t), Some(e)) => {
                    let m = self.solver.ite(cond, t, e);
                ...
                (None, Some(e)) => {
                    self.field_str_arrays.insert(k, e);
                }
                (None, None) => {}
            }
        }
    ...
    pub(super) fn merge_saved_into(&mut self, acc: &mut SavedState, cond: Term, case: &SavedState) {
        ...
        let all_fs: HashSet<_> = case
            .field_str_arrays
            .keys()
            .chain(acc.field_str_arrays.keys())
            .cloned()
            .collect();
        for k in all_fs {
            match (
                case.field_str_arrays.get(&k).copied(),
                acc.field_str_arrays.get(&k).copied(),
            ) {
                (Some(a), Some(b)) if a == b => {}
                (Some(a), Some(b)) => {
                    let m = self.solver.ite(cond, a, b);
                ...
                (Some(a), None) => {
                    acc.field_str_arrays.insert(k, a);
                }
                _ => {}
            }
        }
